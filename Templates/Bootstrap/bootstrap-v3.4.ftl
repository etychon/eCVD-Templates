...
<#-- Determine Ethernet/Cellular WAN interface based on Model - 807, 809, 829, 1101, 1800 -->
  ...
  <#elseif model == "IR182" || model == "IR183">
    <#assign model = "IR1800">
    <#assign ether_if = "GigabitEthernet0/0/0">
    <#-- TODO Add support for multiple cell intfs for IR1800 -->
...
<#--Adding retry delays for PnP work response so that work response does not get missed if modem is reset -->
  ...
  banner login "Cisco IoT Gateway v3.4"
  hostname ${model}_${sn}
...
<#if model == "IR1101">
  ...
  <#list cell_ifs as cell_if>
    ...
    <#-- Initialize weghts for primary ("Cellular 0/0", "Cellular 0/1/0") and secondary cell interfaces ("Cellular 1/0", "Cellular 0/3/0", "Cellular 0/3/0")-->
    ...
    action 150 else
      ...
      action 318 cli command "dialer idle-timeout 0"
      action 320 cli command "dialer-group 1"
      action 321 cli command "zone-member security INTERNET"
      <#if model == "IR1101" || model == "IR1800">
        action 322 cli command "pulse-time 1"
      <#else>
        action 322 cli command "dialer string lte"
        action 323 cli command "encapsulation slip"
      </#if>
    ...
    <#if model == "IR1101" || model == "IR1800">
      <#-- Keep applet saved for devices with pluggable cellular modules. To be triggered via SYSLOG event when cell module plugged in -->
  ...
  action 5 cli command "do delete /f flash:/express-setup-config*"
...
<#else>
  <#--  This block is for non validated dual LTE devices -->
...
<#-- TODO Verify on IR1800 before Enabling-->
<#if model == "IR1101">
  event manager applet cell_module_detected
    ...
    action 055       set cellIntfName "$subslot/0"
    action 060       set cellPnpAppletName "cell_pnp_Cellular$cellIntfName"
    action 070       set gpsEnableAppletName "GPS_ENABLE_IR1100_Cellular$cellIntfName"
    ...
    action 240       string match "*event manager applet GPS_ENABLE_IR1100*" "$_cli_result"
...
<#if model == "IR1101">
  ...
  <#list cell_ifs as cell_if>
    ...
    <#assign i = cell_if?index>
    event manager applet GPS_ENABLE_IR1100_${cell_if?replace(" ", "")}
    <#if all_present_cell_ifs?seq_contains(cell_if)>
    ...
    action 095 if $_regexp_result eq 1
        action 098  puts "Found supported modem with version $_modem_version"
        ...
        action 201 foreach line $_cli_result "\r\n"
          ...
          action 210 if $_regexp_result eq 1
            ...
            action 320 syslog msg "Removing GPS applet event timer"
            action 330 cli command "event manager applet GPS_ENABLE_IR1100_${cell_if?replace(" ", "")}"
            action 335 cli command "event none"
    ...
    action 500 else
      ...
      action 530 syslog msg "Removing GPS applet event timer"
      action 540 cli command "event manager applet GPS_ENABLE_IR1100_${cell_if?replace(" ", "")}"
      action 545 cli command "event none"
...
<#--TODO: Add GPS support for cell modems-->
<#if model == "IR1800">
  event manager applet GPS_ENABLE_IR1800
  event timer watchdog time 60
  action 010 cli command "enable"
  action 020 cli command "show ${cell_ifs[0]} firmware"
      action 030 regexp "Modem is still down, please wait for modem to come up" $_cli_result match
      action 040 if $_regexp_result eq 1
        action 050 syslog msg  "Modem is DOWN, exit without any changes"
        action 060 exit
      action 070 end
  action 200 cli command "show ${cell_ifs[0]} gps"
  action 201 foreach line $_cli_result "\r\n"
    action 202 regexp "^GPS Mode Configured =[ ]+(.+)$" $line match _gps_mode
    action 210 if $_regexp_result eq 1
      action 220 if $_gps_mode eq "not configured"
        ...
        action 222 cli command "conf t"
        action 230 cli command "controller ${cell_contrs[0]}"
        action 240 cli command "lte gps auto-reset"
        action 250 cli command "lte gps mode standalone"
        action 260 cli command "lte gps nmea"
  ...
  event manager applet MgmtTuRecvry
   ...
   action 35   cli command "no ip route $herip 255.255.255.255 ${ether_if} dhcp"
   <#if model == "IR1101">
     <#assign alreadyProcessed = []>
     <#list cell_ifs as cell_if>
       <#if alreadyProcessed?seq_contains(cell_if)>
       ...
       <#if cell_if == "Cellular 0/1/0" || cell_if == "Cellular 0/0" || (cell_if == "Cellular 0/4/0" && model == "IR1800")>
         action 36.${i}   cli command "ip route $newip 255.255.255.255 ${cell_if} $primaryCellIfHerWeight"
       <#else>
         action 36.${i}   cli command "ip route $newip 255.255.255.255 ${cell_if} $secondCellIfHerWeight"
       </#if>
         action 37.${i}   cli command "no ip route $herip 255.255.255.255 ${cell_if}"
         <#assign alreadyProcessed += [cell_if]>
     </#list>
   <#else>
     action 36   cli command "ip route $newip 255.255.255.255 ${cell_ifs[0]} 10"
     action 37   cli command "no ip route $herip 255.255.255.255 ${cell_ifs[0]} 10"
   </#if>
...
<#if model == "IR1101">
  event manager applet checkManagementVpnActive
    event timer watchdog name checkManagementVpnActive time 300 maxrun 60
    action 010  cli command "enable"
    ...
    action 050    puts "Management VPN inactive. Clearing NAT translations and crypto sa"
    action 070    cli command "clear ip nat translation *"
    action 080    cli command "clear crypto sa map Tunnel1-head-0"
    <#-- update cell priorities here via EEM for dual LTE devices -->
    action 090    cli command "event manager run updateRoutesDualLte"
    action 100  end

  event manager applet vpnConnectionDown
    event syslog mnemonic "FLEXVPN_CONNECTION_DOWN"
    action 010    set _vpn_ip ""
    action 020    regexp "Server_public_addr = ([0-9]*\.[0-9]*\.[0-9]*\.[0-9]*)" "$_syslog_msg" match _vpn_ip
    action 030    if $_vpn_ip ne "$herip"
    action 040      exit
    action 050    end
    action 060    cli command "enable"
    <#-- update cell priorities here via EEM for dual LTE devices -->
    action 070    cli command "event manager run updateRoutesDualLte"
  ...
  event manager applet updateRoutesDualLte
    ...
    action 620    end
    !action 630   syslog msg "For interface '$interface' using her weight '$currCellHerWeight' and default gateway weight '$currCellDefaultWeight'"
    ...
    action 755    end
    action 760  end
    action 770  cli command "end"
    action 780  cli command "conf t"
    action 790  cli command "event manager applet updateRoutesDualLte"
    action 800  cli command "event none"
    action 810  cli command "end"
</#if>
end
</#if>
<#else>
