...
event manager applet PERFORM_RECOVERY_ACTIONS
 ...
 !
 ! Reload router when no net & OTOD connectivity outage lasts longer than configured router reload
time
 !
 action 3.2 divide $outage_current $gateway_reboot_time
 action 3.4 if $_remainder eq 0
 action 3.6 syslog msg "Internet connectivity lost for $outage_current min. Attempting gateway
reload"
 ...
 action 4.4 end
 !
 ! Reboot modem when no net & IOTOD connectivity outage lasts longer than configured modem reload
time
 !
 action 4.6 divide $outage_current $modem_reboot_time
 action 4.8 if $_remainder eq 0
 action 5.0 syslog msg "Internet connectivity lost for $outage_current min. Attempting modem
reload"
 action 5.2 cli command "enable"
 action 5.4 cli command "test cellular 0 modem-power-cycle" pattern "#"
 action 5.6 end
 action 5.8 end
 !
 ...
 action 6.3 cli command "show platform nvram | redirect flash:iotd_recovery.log" pattern
"confirm|#"
...
***"
 ...
 action 8.8 cli command "copy flash:/managed/bypass-discovery.cfg startup-config" pattern
"startup-config|#"
